...
def so_dict(db: Session, so) -> dict:
    return {
        ...
        "lines": [{
            ...
            "product_reference": l.product.reference if l.product else None,
            "procurement_strategy": "MTO" if (l.product and l.product.procure_on_demand) else "MTS",
            "procurement_type": l.product.procurement_type if l.product else "",
            "ordered_qty": l.ordered_qty, "delivered_qty": l.delivered_qty,
            "sales_price": l.sales_price, "total": l.total,
            "available_stock": inventory.free_to_use(db, l.product) if l.product else 0,
            "shortage_qty": getattr(l, "shortage_qty", 0) or 0,
            "linked_po": getattr(l, "linked_po", None),
            "linked_mo": getattr(l, "linked_mo", None),
