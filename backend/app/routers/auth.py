...
def _user_public(u: User) -> dict:
    return {"id": u.id, "name": u.name, "login_id": u.login_id, "email": u.email,
            "role": u.role, "position": u.position, "address": u.address,
            "mobile": u.mobile, "photo": u.photo}


@router.post("/signup")
def signup(body: SignupIn, db: Session = Depends(get_db)):
    if not (6 <= len(body.login_id) <= 12):
        raise HTTPException(400, "Login Id must be 6-12 characters")
    ...
    pw = body.password
    if len(pw) <= 8 or pw.lower() == pw or pw.upper() == pw or pw.isalnum():
        raise HTTPException(400, "Password needs >8 chars incl. upper, lower & a special char")
    u = User(name=body.name, login_id=body.login_id, email=body.email,
             password_hash=hash_password(pw), role=body.role, position=body.position)
